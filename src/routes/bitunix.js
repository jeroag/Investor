...
router.post('/partial-close', requireAuth, async (req, res) => {
  try {
    const { symbol, qty, tipo } = req.body;
    if (!symbol || !qty || !tipo)
      return res.status(400).json({ ok: false, error: 'symbol, qty y tipo requeridos' });

    const closeSide = tipo === 'LONG' ? 'SELL' : 'BUY';
    const body = {
      symbol,
    ...
    console.log(`[Bitunix partial-close] OK orderId=${orderId}`);
    res.json({ ok: true, orderId });
  } catch (err) {
    console.error('[Bitunix partial-close]', err.message);
...
router.get('/circuit-breaker', requireAuth, (req, res) => {
  ...
  res.json({ ok: true, active, losses, maxConsec });
});
